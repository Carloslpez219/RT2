...
class RayTracer(object):
    ...
    def rtRayColor(self,intercept,rayDirection,recursion=0):
        ...
        if material.matType == OPAQUE:  
            for light in self.lights:
                if light.lightType=="Ambient":
                    ...
                    if light.lightType=="Directional":
                        ...
                        lightDir = mathOp.subtract_arrays(light.point,intercept.point)
                        lightDir = mathOp.normalizar_vector(lightDir)
            ...
            reflect = reflectVector(intercept.normal,mathOp.deny_array(rayDirection))
            ...
            outside = mathOp.producto_punto(rayDirection,intercept.normal)<0
            bias = mathOp.multiply_scalar_array(0.001,intercept.normal)

            reflect = reflectVector(intercept.normal,mathOp.deny_array(rayDirection))
            reflectOrig = mathOp.add_arrays(intercept.point,bias) if outside else mathOp.subtract_arrays(intercept.point,bias)
            ...
            if not totalInternalReflection(intercept.normal,rayDirection,1.0,material.ior):
                ...
                refractOrig = mathOp.subtract_arrays(intercept.point,bias) if outside else mathOp.add_arrays(intercept.point,bias)
                ...
                reflectColor = mathOp.multiply_scalar_array(Kr,reflectColor)
                refractColor = mathOp.multiply_scalar_array(Kt,refractColor)
    ...
    def rtRender(self):
        ...
        for i, j in indeces:
            ...
            if 0<=x<self.width and 0<=y<self.height:
                ...
                direction = mathOp.normalizar_vector(direction)
